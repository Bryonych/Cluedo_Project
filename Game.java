...
//    }
    ...
    public static void main(String[] args) throws NumberFormatException, IOException{
        ...
        while(!game.gameWon) {
        	for(Character play : players) {
                ...
                else if(game.suggestionMade) {
                	//When it is the turn of the suggester in later rounds
                	if(game.suggestion.getSuggester().equals(play) && game.round > 1) {
                		//when the suggestion is unrefuted, the suggesting player can make an accusation.
                		if(!game.suggestionRefuted) {
                			...
                			System.out.println("Player " + game.refutePlayer + " has refuted your suggestion with:\n" + game.refute.toString());
                		}
                	}
                	//on subsequent players turns
                	else {
                		System.out.println(game.suggestion.toString());
                     	game.refuteSuggestion(play,game.suggestion);
                	}	
                }
                ...
                //player can only make suggestions if no existing suggestions, have not had failed accusations and in a room
                if(!game.suggestionMade && play.canMakeSuggestions() && (play.getCurrentRoom().equals(Cell.Type.KITCHEN) || play.getCurrentRoom().equals(Cell.Type.BALLROOM) || play.getCurrentRoom().equals(Cell.Type.CONSERVATORY) ||
    ...
    public void refuteSuggestion(Character play, Suggestion suggestion) {
    	...
    	if(matches.size() == 1) {
			...
			System.out.println("Suggestion refuted with:\n" + refute.toString());
    	}
